...
class Account:
    ...
    def __repr__(self):
        "*** YOUR CODE HERE ***"
        len_depo = len([depo for depo in self.transactions if depo[0] == 'deposit'])
        len_withdr = len([depo for depo in self.transactions if depo[0] == 'withdraw'])
        return f'Accountholder: {self.holder}, Deposits: {len_depo}, Withdraws: {len_withdr}'
...
def insert(link, value, index):
    ...
    """
    "*** YOUR CODE HERE ***"
    def func(link, value, index, curr):
        if link == Link.empty and curr <= index:
            raise IndexError('Out of bounds!')
        if curr == index:
            link.first, link.rest = value, Link(link.first, link.rest)
        else:
            func(link.rest, value, index, curr + 1)
    func(link, value, index, 0)
