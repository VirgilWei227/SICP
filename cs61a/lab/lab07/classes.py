...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        opponent_card.attack = opponent_card.attack - opponent_card.defense if opponent_card.attack > opponent_card.defense else 0
        opponent_card.defense += opponent_card.defense
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        player.deck = Deck([Card(card.name, card.attack + opponent_card.attack, card.defense + opponent_card.defense) for card in deck.cards])
        opp_deck = opponent.deck.copy()
        opponent.deck = Deck([Card(card.name, card.attack, card.defense) for card in opp_deck.cards if card.attack != opponent_card.attack and card.defense != opponent_card.defense])
        # You should add your implementation above this.
