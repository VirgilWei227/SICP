...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        curr_len = 0
        curr = self.place
        while curr_len != self.min_range and not curr.is_hive:
            curr = curr.entrance
            curr_len += 1
        while self.min_range <= curr_len <= self.max_range and not curr.is_hive:
            if curr.bees:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        self.block_action = self.sting
        self.move_action = self.move_to
        destination = self.place.exit
        self.destination_legel = lambda destination: destination is not None
        if self.slow_times:
            self.block_action = lambda x : None
            self.move_action = lambda destination: self.move_to(destination) if gamestate.time % 2 == 0 else None 
        if self.scare_times:
            destination = self.place.entrance
            self.destination_legel = lambda destination: not destination.is_hive
            if self.scare_times and gamestate.time % 2 == 0:
                self.scare_times -= 1

        if self.blocked():
            self.block_action(self.place.ant)
        elif self.health > 0 and self.destination_legel(destination): 
            self.move_action(destination)
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        self.scare_times = length
        self.is_scaryed = True
        # END Problem EC
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target: Bee):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and not target.is_scaryed:
            target.scare(2)
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not curr.is_hive:
            if curr.ant:
                dict[curr.ant] = dist
                if isinstance(curr.ant, ContainerAnt):
                    if curr.ant.ant_contained:
                        dict[curr.ant.ant_contained] = dist
            if curr.bees:
                for bee in curr.bees:
                    dict[bee] = dist
            curr = curr.entrance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        weak_distance = 0.25 * distance
        waek_insect = 0.0625 * self.insects_shot
        weak = waek_insect + weak_distance
        if self.damage - weak <= 0:
            return 0
        else:
            return self.damage - weak
        # END Problem Optional 2
