...
def get_score(strategy, score_self, score_opponent, 
            dice=six_sided, goal=GOAL_SCORE, say=silence):
    """
    """
    num_rolls = strategy(score_self, score_opponent)
    score_get = take_turn(num_rolls, score_opponent, dice, goal)
    if score_opponent == score_self + score_get:
        new_score_self = score_opponent * 2
    else:
        new_score_self = score_self + score_get
    return new_score_self
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    # if who == 0:
    #     score0 = get_score(strategy0, score0, score1, dice, goal)
    # else:
    #     score1 = get_score(strategy1, score1, score0, dice, goal)
    # who = next_player(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            curr_score = score0 - last_score
            if curr_score > running_high:
                print("%r point(s)! That's a record gain for Player 0!" % curr_score)
                return announce_highest(who, score0, curr_score)
            else:
                return announce_highest(who, score0, running_high)
        else:
            curr_score = score1 - last_score
            if curr_score > running_high:
                print("%r point(s)! That's a record gain for Player 1!" % curr_score)
                return announce_highest(who, score1, curr_score)
            else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while i <= 10:
        ...
        average_point = average_dice(i, dice)
        if average_point > max_point:
            num = i
            max_point = average_point
        i += 1
    return num
    # END PROBLEM 9
...
def run_experiments():
    ...
    print('always_roll(six_side_max) win rate:', average_win_rate(always_roll(six_sided_max)))
...
def final_strategy(score, opponent_score, goal=100):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if hog_pile_strategy(score, opponent_score) == 0:
        return 0
    elif goal - score < 8:
        return 0
    elif goal - score < 10:
